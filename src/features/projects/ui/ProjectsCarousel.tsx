...
const CustomCarouselControls = () => {
  ...
  return (
    <div className="flex items-center justify-end gap-4 mt-8 mr-12">
      <button
        onClick={scrollPrev}
        disabled={!canScrollPrev}
        aria-label="Previous slide"
        className={cn(
          "w-12 h-px bg-gray-300 relative transition-all duration-300 group/btn",
          !canScrollPrev && "opacity-30 cursor-not-allowed",
          canScrollPrev && "hover:bg-[#f18c22] cursor-pointer",
        )}
      >
        <ArrowLeft
          className={cn(
            "absolute -top-2.5 left-0 w-5 h-5 transition-colors",
            canScrollPrev
              ? "text-gray-600 group-hover/btn:text-[#f18c22]"
              : "text-gray-300",
          )}
        />
      </button>
      ...
      <button
        ...
        className={cn(
          ...
          canScrollNext && "hover:bg-[#f18c22] cursor-pointer",
        )}
      >
        <ArrowRight
          className={cn(
            "absolute -top-2.5 right-0 w-5 h-5 transition-colors",
            canScrollNext ? "text-[#f18c22]" : "text-gray-300",
          )}
        />
      </button>
    </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16">
          <SectionHeader
            ...
            description="Real solutions. Real impact. Real results. Projects that make a real difference"
          />
